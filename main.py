...
current_12bit_scale = sc.get_scale_of_12_bit_values(scale_interval=current_scale_interval)
...
cv_prob_menu = m.NumericalValueRangeMenu(
    "CVProb", selected_value=50, increment=20)

steps_menu = m.NumericalValueRangeMenu(
    "Steps", selected_value=number_of_steps, increment=1, min_val=MIN_NUMBER_OF_STEPS, max_val=MAX_NUMBER_OF_STEPS)

octaves_menu = m.NumericalValueRangeMenu(
    "Octaves", selected_value=number_of_octaves, increment=1, min_val=MIN_NUMBER_OF_OCTAVES, max_val=MAX_NUMBER_OF_OCTAVES)
...
def update_sequencer_values() -> None:
    ...
    for submenu in submenus:
        if submenu.name is 'Scale':
            ...
            cv_probability_of_change = submenu.selected_value
        elif submenu.name is 'Steps':
            number_of_steps = submenu.selected_value
        elif submenu.name is 'Octaves':
            number_of_octaves = submenu.selected_value
    current_12bit_scale = sc.get_scale_of_12_bit_values(scale_interval=current_scale_interval, octaves=number_of_octaves)


# loop
while True:
    # todo update sequencer values only when changed
    update_sequencer_values()
    m.loop_main_menu()
    handle_clock_pulse()
